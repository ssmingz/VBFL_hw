...
public class WithoutGenTree {
    ...
    public static void main(String[] args) {
        ...
        for(String pro : pros) {
            ...
            for(int j : bugs3) {
                ...
                String values = String.format("/collected_values/%s/%s_%d/",pro,pro,j);                File valuesDir = new File(values);
                int ms = 0;
                ...
                dependencyParser = null;
                for(int k=0; k<ms; k++) {
                    String log_path = String.format("/collected_values/%s/%s_%d/%d/std.log",pro,pro,j,k);
                    String output_path = String.format("/results/gen_trees/gen_trees-cp_withoutTree/%s/%s_%d/%d", pro,pro,j,k);
                    if (SLICING) {
                        log_path = String.format("/collected_values/%s/%s_%d/%d/std_slicing.log",pro,pro,j,k);
                        output_path = String.format("/results/gen_trees/gen_trees-cp_withoutTree_slicing/%s/%s_%d/%d", pro,pro,j,k);
                    }
                    ...
                    BID = ""+j;
                    MID = ""+k;
                    SRC_BASE = String.format("/d4jsrc/");
                    String pro_cap = Constant.PROJECT_NAME_MAP2.get(pro);
                    TRACE_DIR = String.format("/tracing/info/%s/%s_%d",pro_cap,pro_cap,j);
                    LINESCORE_BASE = String.format("/ochiai_method_results_afterTP/%s/%s_%s/Ochiai_coverage_sd.csv",pro_cap,pro_cap,j);
                    if(pro.equals("lang")||pro.equals("chart")||pro.equals("math")||pro.equals("time")||pro.equals("closure")) {
                        LINESCORE_BASE = String.format("/linear_sd/line/%s/%s_%s/Ochiai_coverage_sd.csv",pro,pro,j);
                    }
                    methodByMID = loadMID(String.format("/collected_values/%s/%s_%s/instrumented_method_id.txt",pro,pro,j));
                    parseClazzMethod(String.format("/collected_values/%s/%s_%s/instrumented_method_id.txt",pro,pro,j));
    ...
    public static void main2(String[] args) {
        ...
        for(String pro : pros) {
            ...
            for(int j : bugs3) {
                dependencyParser = null;
                String values = String.format("/Users//Workspace/FaultLocalization/collected_values/%s/%s_%d/",pro,pro,j);
                ...
                dependencyParser = null;
                for(int k=0; k<ms; k++) {
                    String log_path = String.format("/Users//Workspace/FaultLocalization/collected_values/%s/%s_%d/%d/std.log",pro,pro,j,k);
                    String output_path = String.format("/Users//Workspace/FaultLocalizationresults/gen_trees/gen_trees-cp_withoutTree/%s/%s_%d/%d", pro,pro,j,k);
                    if (SLICING) {
                        log_path = String.format("/Users//Workspace/FaultLocalization/collected_values/%s/%s_%d/%d/std_slicing.log",pro,pro,j,k);
                        output_path = String.format("/Users//Workspace/FaultLocalization/results/gen_trees/gen_trees-cp_withoutTree_slicing/%s/%s_%d/%d", pro,pro,j,k);
                    }
                    File output = new File(output_path);
                    if(output.exists()) {
                        continue;
                    ...
                    BID = ""+j;
                    MID = ""+k;
                    SRC_BASE = String.format("/Users//Workspace/FaultLocalization/EXP/buggyProjects_afterTP/");
                    TRACE_DIR = String.format("/Users//Workspace/FaultLocalization/trace/%s/%s_%d",pro,pro,j);
                    LINESCORE_BASE = String.format("/Users//Workspace/FaultLocalization/ochiai_method_results_afterTP/%s/%s_%s/Ochiai_coverage_sd.csv",pro,pro,j);
                    if(pro.equals("lang")||pro.equals("chart")||pro.equals("math")||pro.equals("time")||pro.equals("closure")) {
                        LINESCORE_BASE = String.format("/Users//Workspace/linear_sd/line/%s/%s_%s/Ochiai_coverage_sd.csv",pro,pro,j);
                    }
                    methodByMID = loadMID(String.format("/Users//Workspace/FaultLocalization/collected_values/%s/%s_%s/instrumented_method_id.txt",pro,pro,j));
                    parseClazzMethod(String.format("/Users//Workspace/FaultLocalization/collected_values/%s/%s_%s/instrumented_method_id.txt",pro,pro,j));
