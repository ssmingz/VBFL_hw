...
public class TestMethodVisitor extends ASTVisitor {
    ...
    public HashSet<MethodDeclaration> traverse() {
        ...
        if(!_uncheckedMethods.isEmpty()) {
            ...
            for(String name : _uncheckedMethods) {
                //JavaLogger.warn(_name + "#traverse Test method not found : " + name + " " + _projectPath);
            }
        }
        return _methodList;
    }
    ...
    public static void insertLoggerInit(TypeDeclaration node, String projectPath) {
        ...
        for(Object f : node.getFields()) {
            if(((FieldDeclaration) f).toString().trim().contains("org.apache.log4j.Logger logger_log4j=org.apache.log4j.Logger.getLogger(\"\");")) {
                alreadyInsert = true;
                break;
            }
        }
        ...
        fd.modifiers().add(_ast.newModifier(Modifier.ModifierKeyword.STATIC_KEYWORD));
    ...
    private boolean checkMethodName(MethodDeclaration node) {
        ...
        if(_methodNames.contains(name)) {
            ...
            //if(!node.getBody().statements().isEmpty()) {
                ...
                // if override, add throwable for the overriden method
                //if(name.contains("testMoreEstimatedParametersSimple")){
                //    name.chars();
                //}
                boolean isOverride = false;
                for(Object mod : node.modifiers()) {
                    if(mod.toString().equals("@Override")) {
                        isOverride = true;
                        break;
                    }
                }
