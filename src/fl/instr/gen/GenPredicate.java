...
public class GenPredicate {
    ...
    public static void genArgumentPred(Expression exp, Statement stmt, ASTNode curMD, int index) {
        ...
        if(castType == null) {
            ...
            return ;
        } else if(castType.isCapture()) {
            return ;
        } else if(castType.isParameterizedType()) {
            return ;
        ...
        switch (stmt.getNodeType()) {
            case Type.IF_STATEMENT:
                ...
                if(exp == oldCond) {
                    ...
                    newCond = assignCond(exp, stmt, curMD, true);
     ...
     * @param stmt
     */
    private static Expression assignCond(Expression exp, Statement stmt, MethodDeclaration curMD, boolean notAssign) {
        Expression newExp;
        if (exp instanceof ParenthesizedExpression) {
            ...
            iop = assignCond(iold, stmt, curMD, notAssign);
            ((ParenthesizedExpression) exp).setExpression(iop);
            newExp = exp;
        } else {
            // 1.cond -> (_count=(cond))
            ...
            if(exp instanceof InfixExpression &&
                ...
                lop = assignCond(lold, stmt, curMD, false);
                ((InfixExpression) exp).setLeftOperand(lop);
                ...
                rop = assignCond(rold, stmt, curMD, false);
                ((InfixExpression) exp).setRightOperand(rop);
                ...
                for(int i=0; i<((InfixExpression) exp).extendedOperands().size(); i++) {
                    ...
                    eop = assignCond(eold, stmt, curMD, false);
                    ((InfixExpression) exp).extendedOperands().set(i, eop);
                }
                //newExp = exp;
            }
