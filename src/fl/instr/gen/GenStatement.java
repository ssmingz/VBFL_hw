...
public class GenStatement {
    ...
    protected static void generateStmt(Expression exp, Statement stmt, int lineNo, int genAfter) {
        boolean ignoreAssign = false;
        if (stmt instanceof IfStatement || stmt instanceof WhileStatement || stmt instanceof DoStatement) {
            ignoreAssign = true;
        }
        String content = "";
        ...
        if (exp instanceof SuperFieldAccess) {
            ...
            GenWriter.write(content, exp, stmt, genAfter);
        } else if (exp instanceof Assignment && !ignoreAssign) {
