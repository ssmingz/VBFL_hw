...
//            vertexes.put(endNode.getVertexId(), endNode);
        ...
        //check def-use edge to split the dependency chain
        DependencyGraphVertex start = vertexes.get(startNode.getVertexId());
        if (edgeType == EdgeType.DEF_USE) {
            for (DependencyGraphEdge edge : start.endEdges) {
                if (edge.getEdgeType() == EdgeType.DEF_USE) {
                    start = edge.getStartVertex();
                }
            }
        }
        start.connect(vertexes.get(endNode.getVertexId()), edgeType);
        return true;
    }
    ...
    public boolean equals(Object o) {
        ...
        return DependencyGraphVertex.checkSetEqual(vertexes.entrySet(), that.vertexes.entrySet());
    }
