...
public class CommandUtil {
    ...
    public static List<Map<String[], List<String[]>>> getDependency(String passPath, String irFilePath,
        ...
        // clang-12 -O0 -g -emit-llvm -fprofile-generate -fsyntax-only -S test.c -o
        // test.bc
        // opt -load libDependencyPass.so -world test.c -o test_new.bc
        stringBuilder.append("clang-12 -O0 -g -emit-llvm -fprofile-generate -fsyntax-only -S " + sourceFilePath + " -o "
                + irFilePath);
        ...
        // empty result obtained when execution error occured, this lead to error before
        // gengraph.
    ...
    private static List<Map<String[], List<String[]>>> parseedge(List<String> result) {
        ...
        for (int i = 0; i < result.size(); i++) {
            if (result.get(i).equals("")) {
                ...
                // if (isStoreDefuse && edgevertex[6].equals("from") &&
                // edgevertex[7].equals("def-use"))
                // continue;
                List<String> vertexlist = new ArrayList<>();
                i = getVertex(i, result, vertexlist);
