            ...
            with open(prior_gcov, 'r') as f:
                for l in f:
                    ...
                    if lineNo > 0:
                        ...
                        if precov != 0 and lineNo not in allcovByLine:
                            allcovByLine.append(lineNo)
                ...
                for l in f:
                    ...
                    if lineNo > 0:
                        ...
                        if bugcov.isdigit():
                            ...
                            bugcov = 0
                        if bugcov != 0:
                            gt_line = int(bugs[int(id)-1].bug_line_no.split(':')[0])
                            if lineNo not in allcovByLine:
                                allcovByLine.append(lineNo)
                            if precovByLine[lineNo] != bugcov and lineNo not in bugcovByLine:
                                start_end = get_start_end_line(lineNo)
                                if start_end[0] <= gt_line <= start_end[1]:
                                    print("line:" + str(lineNo))
                                    print("prevcov:" + str(precovByLine[lineNo]))
                                    print("bugcov:" + str(bugcov))
                                bugcovByLine.append(lineNo)
                        
        start_end_dict = dict()
        for bugLine in bugcovByLine:
            start_end_dict[bugLine] = get_start_end_line(bugLine)
        def compute_cov_score(currentLine):
            # print("line:" + str(currentLine))
            # print(bugcovByLine)
            # print(allcovByLine)
            fail_test_cov_line = 0
            all_test_cov_line = 0
            ...
            if start_line != 0:
                gt_line = int(bugs[int(id)-1].bug_line_no.split(':')[0])
                for i in range(currentLine + 1, end_line + 1):
                    if i in bugcovByLine:
                        fail_test_cov_line += 1
                    if i in allcovByLine:
                        all_test_cov_line += 1
                if start_line <= gt_line <= end_line:
                    print("line:" + str(currentLine))
                    print("fline:" + str(fail_test_cov_line))
                    print("aline:" + str(all_test_cov_line))
            else:
                return 0
            # print(fail_test_cov_line)
            # print(all_test_cov_line)
            return fail_test_cov_line / all_test_cov_line if all_test_cov_line != 0 else 0

        totalByLine = {lineNo:compute_cov_score(lineNo) for lineNo in allcovByLine}
        # print(totalByLine)
...
if __name__ == '__main__':
    # avail = [i for i in range(63, 111) if i not in[11, 15, 18, 27, 47, 54, 70, 84]]
    avail=[63, 64, 66, 67, 71, 72, 76, 77, 78, 79, 82, 83, 85, 87, 91, 94, 95, 96, 97, 98, 99, 101, 102, 104, 107, 108, 109]
